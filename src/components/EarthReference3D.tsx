...
function Scene({ rE, rP, planetRadius, color, showTicks }: { rE: number; rP: number; planetRadius: number; color: string; showTicks: boolean }) {
  ...
  useFrame((state, dt) => {
    ...
    if (earthRef.current) {
      ...
      earthRef.current.rotation.y += dt * 0.12
    }
    if (planetRef.current) {
      planetRef.current.scale.setScalar(s.p)
      planetRef.current.position.set(xP, s.p, 0)
      planetRef.current.rotation.y += dt * 0.12
    }
    if (labelERef.current) labelERef.current.position.set(xE, 2 * s.e + 0.34, 0)
    if (labelPRef.current) labelPRef.current.position.set(xP, 2 * s.p + 0.34, 0)
    if (baseRef.current) {
      baseRef.current.scale.x = worldW
      baseRef.current.position.set(cx, -0.015, 0)
    }
    // 刻度池：落在当前基线范围内的才可见（对数档全隐）
    for (let i = 0; i <= TICK_MAX - TICK_MIN; i++) {
      const v = TICK_MIN + i
      const m = tickRefs.current[i]
      if (m) m.visible = showTicks && v > left + 0.4 && v < right - 0.4
    }
      ...
      {Array.from({ length: TICK_MAX - TICK_MIN + 1 }, (_, i) => {
        ...
        return (
          <mesh key={v} ref={(el) => { tickRefs.current[i] = el }} position={[v, 0.07, 0]} visible={false}>
            ...
            <meshBasicMaterial color="#59627a" />
          </mesh>
        )
      })}
      <mesh ref={earthRef}>
        <sphereGeometry args={[1, 48, 48]} />
        <meshStandardMaterial color="#d8dde8" roughness={0.82} metalness={0} />
      </mesh>
      <mesh ref={planetRef}>
        <sphereGeometry args={[1, 48, 48]} />
    ...
    <div style={{ borderBottom: `1px solid ${THEME.panelBorder}`, padding: '10px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <span style={{ fontFamily: THEME.displayFont, fontSize: '0.62rem', letterSpacing: '0.2em', color: THEME.accentCyan, textShadow: THEME.labelGlow }}>
            EARTH REFERENCE
          </span>
          <span style={{ fontSize: '0.66rem', letterSpacing: '0.1em', color: THEME.textFaint }}>地球参照系</span>
        </div>
